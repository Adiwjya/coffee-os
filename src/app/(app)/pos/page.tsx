...
export default function POSPage() {
  ...
  function handleCheckout() {
    ...
    toast.success("Transaksi berhasil!", {
      description: `${result.transaction!.id} • ${formatRupiah(
        result.transaction!.totalAmount
      )}`,
    });
  }

  return (
    <div className="grid h-[calc(100vh-7rem)] grid-cols-1 gap-4 lg:grid-cols-[1fr_380px]">
      {/* Product catalog */}
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-3">
          {cartDetailed.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-2 text-center text-muted-foreground">
              ...
              <p className="text-xs">Ketuk menu di sebelah kiri.</p>
            </div>
          ) : (
